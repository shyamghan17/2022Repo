...
class CompletedServices extends Component {
  ...
  fetchCompletedService = async () => {
    ...
    if (storedType == "freelancer") {
      ...
      const response = await fetch(
        CONSTANT.BaseUrl +
          "dashboard/get_employer_services?type=completed&user_id=" +
          Pid
      );
      const json = await response.json();
      if (
          ...
          <View style={styles.footer}>
            ...
            >
              <Text style={styles.btnText}>
                {CONSTANT.CompletedServicesLoadMore}
              </Text>
          ...
          <FlatList
            ...
            renderItem={({ item, index }) => (
              <TouchableOpacity
                activeOpacity={0.7}
                onPress={() =>
                  this.props.navigation.navigate("CompleteServicesDetail", {
              ...
              >
                {storedType == "freelancer" ? (
                  <CompletedServicesCard
                    ...
                    employer_title={`${entities.decode(item.employer.employer_title)}`}
                    employertagline={`${entities.decode(item.employer.employertagline)}`}
                  ...
                  <CompletedServicesCard
                    ...
                    employertagline={`${entities.decode(
                      item.freelancertagline
                    )}`}
                    service_title={`${entities.decode(item.service_title)}`}
                    price={`${entities.decode(item.price)}`}
                    featured_img={{
