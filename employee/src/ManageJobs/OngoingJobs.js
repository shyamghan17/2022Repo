...
class OngoingJobs extends Component {
  ...
  fetchOngoingJob = async () => {
    ...
    if(storedType == "freelancer"){
      ...
      console.log('fetchOngoingJobs', this.state.fetchOngoingJobs)
    }else{
      const response = await fetch(
  ...
  render() {
    ...
    return (
      <View style={styles.container}>
        <SimpleHeader HeaderText={CONSTANT.ManageJobsOngoingJobs}/>
        {isLoading && (
        ...
        {this.state.fetchOngoingJobs.length < 1 ? 
          ...
          <View style={{flex: 1,}}>
            <View style={[styles.section, styles.ManageJobsProjectsInfoArea, {justifyContent:'space-between'}]}>
              <Text style={styles.MainHeadingTextStyle}>{CONSTANT.ManageJobsOngoingProjects}</Text>
